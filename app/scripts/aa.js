...
.directive("auto",function(){
	return{
		restrict:'AEMC', 
		template:'<div class="syy"><li ng-repeat="s in qq">{{s.title | create}}</li><li ng-repeat="y in ee">{{y.title | create}}</li></div><div class="center"><img  ng-repeat="x in ww" ng-src="{{x.img}}" /></div>',
		scope:true,
		transclude: true,
